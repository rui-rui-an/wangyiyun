  ...
  <div class="wrap">
    <div class="play_wrap" id="player">
      ...
      <songlist ref="songlist"></songlist>
       <centercd ref="centercd"></centercd>
       <commentslist ref="commentslist"></commentslist>
      </div>
      <div class="audio_con">
        <audio :src="songurl" @play="play" @pause="pause" controls autoplay loop class="myaudio"></audio>
      </div>
    </div>
  </div>
</template>
...
import centercd from './components/centercd'
export default {
  components:{
    songlist,
    commentslist,
  ...
  methods: {
    search(){
      axios({
        ...
        this.$refs.songlist.list = data.data.result.songs
        this.$refs.songlist.myIscrollRefresh()
        // console.log(data);
      }).catch((err)=>{
        console.log(err);
      })
    ...
    getSongInfo(id){
      ...
      axios({
        ...
        // this.songurl = data.data.data[0].url
        // console.log( this.songurl);
        this.$refs.commentslist.commments = data.data.comments
        this.$refs.commentslist.getmyscroll()
      }).catch((err)=>{
        console.log(err);
      }),
      ...
      axios({
        ...
        // console.log(data);
        this.$refs.centercd.picurl = data.data.songs[0].al.picUrl
        // console.log(this.$refs.centercd.picurl);
      }).catch((err)=>{
        console.log(err);
      })
    },
    play(){
      this.$refs.centercd.playing = true
    },
    pause(){
      this.$refs.centercd.playing = false
    }
  },
}
